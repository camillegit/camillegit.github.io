...
On admet alors que les probabilités nous amènent à former un langage artificiel qui peut nous sembler pertinent, cependant quelle sémantique cela peut-il véhiculer et comment la signification et la probabilité s'articulent dans l'évolution d'une langue ?
...
Cette mathématisation de la communication, avec la conceptualisation des n-grams par Claude Shannon, amène à des développements spectaculaires. Les modèles de langage probabilistes deviennent, à partir de là, un champ de recherche autonome au croisement de la linguistique, de l’informatique et de la statistique. Chris Manning et Hinrich Schütze relatent cette approche statistique du langage dans *Foundations of Statistical Natural Language Processing*. Ils montrent comment l’entropie et la modélisation probabiliste sont utilisées pour capturer la structure des langues, à travers les n-grams comme élément fondateur.

Le passage aux réseaux neuronaux constitue une étape cruciale. En remplaçant les comptages explicites par des représentations contenues dans les paramètres ou poids du réseau, il devient possible d’exploiter des régularités plus subtiles dans les données. Ici encore, la notion d’entropie demeure un outil essentiel pour évaluer la capacité d’un modèle à réduire l’incertitude sur la séquence suivante. L’apprentissage consiste toujours, en un sens, à comprimer le signal en conservant l’information la plus pertinente. Puis c’est avec l’introduction du mécanisme d’attention que le champ bascule définitivement. Dans *Attention Is All You Need*, une nouvelle structure de réseau de neurone appellée *transformer* remplace les architectures séquentielles par un système d’attention qui pondère chaque mot en fonction de sa contribution à la prédiction du mot suivant. D’un point de vue conceptuel, cette pondération rappelle directement la distribution statistique du langage décrite par Shannon : certaines unités lexicales possèdent une probabilité plus forte dans un contexte donné, et l’attention fonctionne comme une généralisation de cette idée, où la probabilité conditionnelle est remplacée par une mesure de pertinence contextualisée et apprise. Les grands modèles de langage (LLM comme GPT ou LLaMa) contemporains héritent ainsi de cette trajectoire intellectuelle. De Shannon à Vaswani, le fil conducteur reste l'estimation et l'exploitation les distributions probabilistes du langage. 

Source principale :

Claude Shannon, Warren Weaver (1948) *Théorie mathématique de la communication*

Autres lectures :

Chris Manning, Hinrich Schütze (1999) *Foundations of Statistical Natural Language Processing*
